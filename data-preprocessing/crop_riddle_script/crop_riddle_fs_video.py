...
# Function to read end intruction in the root directory
def readEndInstructionFile():
    """
    Read instruction file
    """
    with open('ends.json', 'r') as ends:
        data = json.load(ends)
    return data
...
# Check json file
print("Checking start instruction file...")
check_json_file("ins.json")

print("Checking end instruction file...")
check_json_file("ends.json")
# Read instructions from file

data = readInstructionFile() 

end_data = readEndInstructionFile()
...
video_paths = [os.path.join(root_path, f) for f in os.listdir(root_path) if f.endswith('.mp4')]
...
if not os.path.exists(cropped_video_destination_path):
    ...
    permission = input(f"Do you want to use the existing folder:\ {cropped_video_destination_path}  \ for the cropped videos [y or n]: ") 
    if permission.lower() == "y":
        print("Going ahead with the script...")
    else:
        raise ValueError("Please check destination folder of cropped videos in the \ instructions file")
...
def __main__(args):
    ...
    for i, video_path in enumerate(video_paths):
        ...
        for segment in result["segments"]:
            ...
            found = False
            target_end_statements = end_data["target_statements"]

            if not found:
                for end_statement in target_end_statements:
                    # search for end statement
                    if re.search(end_statement, text, re.IGNORECASE):
                        new_end = end-(0.19*duration - cropped_video.duration)
                        print(end)
                        print(0.19*duration)
                        print(new_end)
                        print(cropped_video.duration)
                        if new_end > 0:
                            cropped_video = cropped_video.subclip(0, new_end)
                            found = True
        ...
        # Save cropped video
        cropped_video.write_videofile(f"{cropped_video_destination_path}/{filename}_cropped.mp4", fps=video.fps)  
