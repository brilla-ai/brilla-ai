...
class AIOperationsService(IAIOperationsService):
    def __init__(self, ai_operations_repository: Annotated[AIOperationsRepository, Depends(AIOperationsRepository)], command_manager : Annotated[ConnectionManager, Depends(get_connection_manager)]):
        self.ai_operations_repository = ai_operations_repository
    ...
    async def update_ai_operations(self, id: uuid4, ai_operations : AIOperationsUpdateModel):
            ...
            ai_operations_response  =   self.ai_operations_repository.update_ai_operations(id, ai_operations_dict)
            
            response = BaseResponseModel.create_response(ai_operations_response, "Ai operations updated successfully", status.HTTP_200_OK)
    ...
    def get_ai_operation(self):
            ai_operations_response  =   self.ai_operations_repository.get_ai_operation()
            print( ai_operations_response )
            return BaseResponseModel.create_response(ai_operations_response, "Ai operations fetched successfully", status.HTTP_200_OK)
