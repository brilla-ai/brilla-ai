...
def intermediary_to_schema(tables, relationships, output):
    pass
    # """ Transforms and save the intermediary representation to the file
    # chosen. """
    # dot_file = _intermediary_to_dot(tables, relationships)
    # graph = AGraph()
...
def filter_resources(
    ...
    Exclude the following:
        1. Tables and relationships with tables present in the
        exclude_tables (lst of str, tables names)
        2. Columns (of whichever table) present in the exclude_columns (lst
        of str, columns names)
    Disclosure note:
...
def render_er(
    ...
    :param mode: str in list:
        ...
        'auto': choose from the filename:
            '*.er': writes to a file the markup to generate an ER style
            diagram.
            '.dot': returns the graph in the dot syntax.
            else: return a graph to the format graph
    :param include_tables: lst of str, table names to include, None means
    include all
    :param include_columns: lst of str, column names to include, None means
    include all
    :param exclude_tables: lst of str, table names to exclude, None means
    exclude nothing
